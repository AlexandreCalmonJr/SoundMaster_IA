...
(function () {
    ...
    function _selectSceneByIndex(index) {
        ...
        pm._setHTML('sb-preview-content', '<div class="w-full space-y-3">' + (_selectedScene.description ? '<p class="text-sm text-slate-400">' + _escapeHtml(_selectedScene.description) + '</p>' : '') + '<div class="grid grid-cols-3 gap-3"><div class="bg-slate-800/40 rounded-xl p-3 text-center"><div class="text-xs text-slate-500 mb-1">Estilo</div><div class="text-sm font-bold text-cyan-400">' + _escapeHtml(_selectedScene.genre || '--') + '</div></div><div class="bg-slate-800/40 rounded-xl p-3 text-center"><div class="text-xs text-slate-500 mb-1">Canais</div><div class="text-sm font-bold text-cyan-400">' + (_selectedScene.channels || 16) + '</div></div><div class="bg-slate-800/40 rounded-xl p-3 text-center"><div class="text-xs text-slate-500 mb-1">Mix</div><div class="text-sm font-bold text-cyan-400">' + (_selectedScene.mixType || 'Estereo') + '</div></div></div>' + (_selectedScene.aiData ? '<div class="bg-purple-900/20 border border-purple-500/20 rounded-xl p-3 text-xs text-slate-400"><strong class="text-purple-400">IA:</strong> ' + _escapeHtml(_selectedScene.aiData) + '</div>' : '') + '</div>');
